...
class DetailActivity : AppCompatActivity() {
        ...
        findViewById<Button>(R.id.watchVideo).setOnClickListener {
            CoroutineScope(Dispatchers.Main).launch {
                if (playerUrl != null) {
                    ...
                    Toast.makeText(this@DetailActivity, "Фильм не найден.", Toast.LENGTH_SHORT).show()
                }
            }
        }
    ...
    private fun checkMovieInFavorites(userId: String, movieId: String, heartImageView: ImageView) {
        ...
        userRef.addListenerForSingleValueEvent(object : ValueEventListener {
            ...
            override fun onCancelled(databaseError: DatabaseError) {
                Toast.makeText(this@DetailActivity, "Ошибка при проверке фильма: ${databaseError.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
    ...
    private fun removeMovieFromFavorites(userId: String, movieId: String) {
        ...
        userRef.child("movies").child(movieId).removeValue()
            ...
            .addOnFailureListener { e ->
                Toast.makeText(this, "Ошибка при удалении фильма: ${e.message}", Toast.LENGTH_SHORT).show()
            }
    }
    ...
    private fun saveMovieToFavorites(userId: String, movieId: String) {
        ...
        userRef.child("movies").child(movieId).setValue(true)
            ...
            .addOnFailureListener { e ->
                Toast.makeText(this, "Ошибка при сохранении фильма: ${e.message}", Toast.LENGTH_SHORT).show()
            }
    }
    ...
    private fun sendRequest(idFilm: String) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val countries = document.select("div.right-info-element .info-element-content.country")
                    .map { it.text().trim(',') }

                val actorsElement = document.select("div.right-info-element.actors").firstOrNull()
                val actorsText = actorsElement?.ownText()?.trim()
                val actorsList = (actorsText?.split(", ")?.joinToString(", ") { it.trim() } ?: "") + " другие"

                val directorElements = document.select("div.right-info-element #director-value .zamanushka-actors")
                ...
                val durationOrSeason = if (isFilm) {
                    ...
                    val seasonInfo = document.select("div.kino-card-quality-badge.top-poster-text").attr("title")
                    seasonInfo.split(" ").take(2).joinToString(" ").takeIf { it.isNotEmpty() }
                }

                movieTimeTxt.visibility = if (durationOrSeason.isNullOrEmpty()) View.GONE else View.VISIBLE
                ...
                withContext(Dispatchers.Main) {
                    ...
                    recycleViewCategory.layoutManager = LinearLayoutManager(this@DetailActivity, LinearLayoutManager.HORIZONTAL, false)

                    val countryAdapter = CategoryEachFilmListAdapter(countries)
                    recycleViewCountry.adapter = countryAdapter
                    recycleViewCountry.layoutManager = LinearLayoutManager(this@DetailActivity, LinearLayoutManager.HORIZONTAL, false)
                ...
                CoroutineScope(Dispatchers.Main).launch {
                    ...
                    Log.d("MovieYearCon", "$currentYear, ${movieYearCountry.text}")
                }
                withContext(Dispatchers.Main) {
                ...
                Log.e("sendRequest", "Error fetching data: ${e.message}")
                withContext(Dispatchers.Main) {
                    progressBar.visibility = View.GONE
                    scrollView.visibility = View.VISIBLE
                }
    ...
    private fun extractTrailerUrlFromScript(url: String) {
        GlobalScope.launch(Dispatchers.IO) {
            try {
                ...
                for (script in scripts) {
                    ...
                    if (matcher.find()) {
                        ...
                        withContext(Dispatchers.Main) {
                            ...
                            Log.d("TrailerURL", "Загрузка трейлера: $trailerUrl")
                ...
                Log.e("Error", "Не удалось извлечь URL трейлера: ${e.message}")
            }
        }
    }
    ...
    private fun initView() {
        ...
        recycleViewCountry.layoutManager = LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false)

        backImg.setOnClickListener { finish() }
    }

    private fun openVideoPlayer(url: String) {
        val intent = Intent(this, VideoPlayerTrailerActivity::class.java)
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (playerUrlExists) {
                    ...
                    Log.d("PlayerCheck", "Player exists on the page: $url, Player URL: $playerUrl")
                } else {
                    Log.d("PlayerCheck", "No player found on the page: $url")
                }

                playerUrlExists
            } catch (e: Exception) {
                Log.e("CheckFilmLink", "Error checking link: ${e.message}")
                false
            }
        }
    }
