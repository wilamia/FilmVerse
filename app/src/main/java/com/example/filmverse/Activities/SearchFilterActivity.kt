...
class SearchFilterActivity : AppCompatActivity() {
    ...
    private fun initializeViews() {
        noResultsTextView = findViewById(R.id.textView27)
    ...
    private fun setupCheckboxHeaders(headerView: View) {
        ...
        setupHeader(headerView, R.id.header5, R.id.checkbox_container5) { setupCountryCheckboxes(it) }
    }
    ...
    private fun sendRequestFilms(page: Int, url: String) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                Log.d("SearchFilterActivity", "Fetching data from: $newUrl")

                val doc: Document = Jsoup.connect(newUrl).userAgent("Mozilla/5.0").get()
                Log.d("SearchFilterActivity", "Fetching data from: $doc")
                ...
                if (movieElements.isEmpty()) {
                    Log.d("SearchFilterActivity", "No movie elements found!")
                    withContext(Dispatchers.Main) {
                        clearMovieList()
                        updateUI()
                    }
    ...
    private fun extractMovieData(element: org.jsoup.nodes.Element) {
        ...
        if (title.isNotEmpty()) {
            val posterPath = element.select("img").attr("data-src").ifEmpty { element.select("img").attr("src") }
            ...
            Log.d("SearchFilterActivity", "Extracted movie: Title: $title, Poster URL: $posterUrl, ID: $id")
        } else {
            Log.e("SearchFilterActivity", "Movie title is empty!")
        }
    }
    ...
    private suspend fun updateUI() {
        ...
        if (movies.isEmpty()) {
            noResultsTextView.visibility = View.VISIBLE
            recyclerViewFilmSearch.visibility = View.GONE // Скрываем RecyclerView
        } else {
    ...
    private suspend fun handleError(e: Exception) {
        withContext(Dispatchers.Main) {
            loading.visibility = View.GONE
            isLoading = false
            Log.e("SearchFilterActivity", "Error fetching data: ${e.message}", e)
        }
    }
    ...
    private fun buildUrl(): String {
        ...
        // Gather selected countries and encode
        val selectedCountries = getSelectedCheckboxesFromContainer(R.id.checkbox_container5).joinToString(",") { it.encodeURL() }
        if (selectedCountries.isNotEmpty()) {
            urlParams["countries"] = selectedCountries
        }

        // Gather genres and encode
        val genres = getSelectedCheckboxesFromContainer(R.id.checkbox_container2).map { it.encodeURL() }
        if (genres.isNotEmpty()) {
            urlComponents.add("genres=${genres.joinToString(",")}")
        }

        // Example for gathering years
        ...
        // Example for gathering type
        val type = getType(getSelectedCheckboxesFromContainer(R.id.checkbox_container3).joinToString())?.encodeURL() ?: ""
        if (type.isNotEmpty()) {
            urlComponents.add("x.type=$type")
        }
        // Example for gathering sort options
        val sortOption = getSelectedSortOption(getSelectedCheckboxesFromContainer(R.id.checkbox_container4).joinToString())?: "date;desc"
        urlComponents.add("sort=$sortOption")

        // Construct the final URL
        val urlBuilder = StringBuilder(baseUrl)
        urlParams.forEach { (key, value) ->
            urlBuilder.append(if (urlBuilder.length > baseUrl.length) "/" else "").append("$key=$value")
        }

        if (urlComponents.isNotEmpty()) {
            urlBuilder.append(if (urlBuilder.length > baseUrl.length) "/" else "").append(urlComponents.joinToString("/"))
        }

        val completeUrl = urlBuilder.toString()
        Log.d("SearchFilterActivity", "Constructed URL: $completeUrl")
        return completeUrl
    }
    ...
    private fun getSelectedCountryOption(selectedCountry: String): String? {
    ...
    private fun setupHeader(headerView: View, headerId: Int, containerId: Int, setupFunction: (LinearLayout) -> Unit) {
    ...
    private fun setupGenreCheckboxes(container: LinearLayout) {
        val genres = listOf("Биография", "Боевик", "Вестерн", "Военный", "Детектив", "Детский", "Документальный", "Драма", "История", "Комедия",
            "Криминал", "Мелодрама", "Мистика", "Мюзикл", "Приключения", "Семейный", "Спорт", "Триллер", "Телепередачи", "Фантастика", "Фэнтези", "Ужасы")
        val gridLayoutGenres = createGridLayout()

        genres.forEach { genre ->
            val checkBox = createCheckbox(genre)
            checkBox.setOnCheckedChangeListener { _, isChecked ->
                Log.d("SearchFilterActivity", "$genre is checked: $isChecked")
            }
    ...
    private fun setupTypeCheckboxes(container: LinearLayout) {
        val types = listOf("Фильм", "Сериал", "Мультфильм", "Мультсериал", "Дорама (Фильм)",
    ...
    private fun setupSortCheckboxes(container: LinearLayout) {
        val sorts = listOf("По дате", "По КП", "По IMDB", "По оценкам", "По алфавиту",
    ...
    private fun setupCountryCheckboxes(container: LinearLayout) {
        val countries = listOf("Австралия", "Аргентина", "Бельгия", "Бразилия", "Великобритания", "Германия", "Гонконг", "Индия", "Ирландия",
            "Исландия", "Испания", "Италия", "Казахстан", "Канада", "Китай", "Колумбия", "Корея Южная", "Люксембург", "Мексика",
            "Нидерланды", "Никарагуа", "Новая Зеландия", "Норвегия", "ОАЭ", "Польша", "Россия", "Румыния", "СССР", "США", "Таиланд",
            "Тайвань", "Турция", "Украина", "Франция", "Хорватия", "Чехия", "Чили", "Швейцария", "Швеция", "Эстония", "ЮАР", "Япония")
